          ...
          {plans.map((plan, index) => (
            <article
              ...
              className={`flex flex-col rounded-2xl p-7 transition-all duration-500 hover:-translate-y-1 ${
                plan.highlighted
                  ? "bg-surface-darker text-white shadow-elegant relative"
            ...
            >
              ...
              <p className={`text-sm ${plan.highlighted ? "text-white/65" : "text-muted-foreground"}`}>
                {plan.description}
              </p>

              <div className="mt-6 mb-6 flex items-baseline gap-1.5">
                <span className="font-serif text-5xl tracking-tight">{plan.price}</span>
                <span className={`text-xs ${plan.highlighted ? "text-white/55" : "text-muted-foreground"}`}>
                  {plan.period}
                </span>
              </div>

              <ul className={`space-y-3 text-sm border-t pt-6 ${plan.highlighted ? "border-white/10" : "border-border"}`}>
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2">
                    <span className={`mt-2 inline-block size-1.5 rounded-full ${plan.highlighted ? "bg-accent" : "bg-foreground"}`} aria-hidden="true" />
                    <span>{f}</span>
                  </li>
                ))}
              </ul>
              ...
              <a
                ...
                className={`mt-8 inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-medium transition-transform duration-300 hover:scale-[1.02] ${
                  plan.highlighted ? "bg-accent text-accent-foreground" : "bg-foreground text-background"
                }`}
              >
                {plan.cta}
              </a>
